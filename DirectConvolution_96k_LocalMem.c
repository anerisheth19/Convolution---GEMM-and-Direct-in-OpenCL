...
// Problem 4  - Direct Convolution in OpenCL for SqueezeNet with local memory
//Written by Aneri Sheth - 801085402
//Reference taken from Lecture Slides by Dr. Tabkhi and code given by TA Arnab Purkayastha
//Reference: https://github.com/grantbrown/OpenCL-Seminar/blob/master/hw5/convolution.cl and https://github.com/TartanLlama/cl-convolution 
...
void read_filters(int *filters)
{
	...
	FILE *fp;	
   	char num_buff[255];
	double n;
   	fp = fopen("snweights-ints3.txt", "r"); //read mode
	int i=0;
...
int main(int argc, char** argv) {
	...
	int* filter = (int*) malloc(96*K*K*sizeof(int));
	...
	unsigned char* output_r = (unsigned char*) malloc(HEIGHT * WIDTH * sizeof(unsigned char));
        unsigned char* output_g = (unsigned char*) malloc(HEIGHT * WIDTH * sizeof(unsigned char));
        unsigned char* output_b = (unsigned char*) malloc(HEIGHT * WIDTH * sizeof(unsigned char));
  ...
  // Create a compute context 
   ...
   lFileSize = LoadOpenCLKernel("DirectConvolution_96k_LocalMem.cl", &KernelSource);
   if( lFileSize < 0L ) {
   ...
   while(imagecount<120) {
    ...
    d_filter = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 96*K*K*sizeof(int), filter, &err);
    ...
    err |= clEnqueueWriteBuffer(commands, d_filter, CL_TRUE, 0, 96*K*K*sizeof(int), filter, 0, NULL, NULL);   
   ...
   int cols = WIDTH;
   int filtersize = K;
     //do this for all 96 kernels
     while (filter_count < 96) {
