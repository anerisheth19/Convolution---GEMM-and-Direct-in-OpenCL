...
// Problem 2  - Direct Convolution with local memory in OpenCL
...
int main(int argc, char** argv) {
	...
	unsigned char* image_r = (unsigned char*) malloc(HEIGHT * WIDTH * sizeof(unsigned char));  //R
	unsigned char* image_g = (unsigned char*) malloc(HEIGHT * WIDTH * sizeof(unsigned char)); //G
	unsigned char* image_b = (unsigned char*) malloc(HEIGHT * WIDTH * sizeof(unsigned char)); //B
	int a;
	...
	cl_float kernelExecTimeNs; //measure time
 	...
 	unsigned char* output_r = (unsigned char*) malloc(HEIGHT * WIDTH * sizeof(unsigned char)); //R output
	  ...
	  // Create a compute context 
	   ...
	   lFileSize = LoadOpenCLKernel("DirectConvolution_LocalMem.cl", &KernelSource);
	   if( lFileSize < 0L ) {
