...
/* Kernel for GEMM with local memory - Squeezenet*/ 
...
__kernel void convolute(__global unsigned char* output, __global unsigned char* image, __global unsigned char* filter, int filter_size, int rows, int cols, int kf)
{
   ...
   for(i = localRow; i < 8; i+=get_local_size(1)) {
	int currRow = g2 + i;
	for(j = localCol; j < 9; j+=get_local_size(0)) {
		int currCol = g1 + j;
		if(currRow < rows && currCol < cols) {
			local_mem_block[i * 8 + j] = image[currRow * cols + currCol];
		}
	}
   } 
